...
class AddressSearchDialog(ctk.CTkToplevel):
    def __init__(self, parent, headers: dict, title="주소 찾기", hint=""):
    ...
    def _build(self, hint):
        ctk.CTkLabel(self, text=hint or "주소를 입력하고 검색하세요",
                     font=ctk.CTkFont(size=13, weight="bold"),
                     wraplength=490).pack(pady=(18, 8), padx=20)

        row = ctk.CTkFrame(self, fg_color="transparent")
        row.pack(fill="x", padx=20)
        self.entry = ctk.CTkEntry(row, placeholder_text="예) 광주 북구 ○○로 ○○",
                                  width=340, height=36)
        ...
        self.lb = tk.Listbox(lf, font=("맑은 고딕", 10),
                             selectbackground="#2E86C1", activestyle="none",
    ...
    def _search(self):
        ...
        if self._items:
            ...
            self.status.configure(text=f"{len(self._items)}건 — 원하는 주소를 선택하세요")
        else:
...
class AddressFixDialog(ctk.CTkToplevel):
    ...
    def _build(self, name, orig, rev):
        ...
        for label, text, bg in [
            ...
            ctk.CTkLabel(body, text=label, font=ctk.CTkFont(size=11),
        ...
        ctk.CTkLabel(body, text="두 주소가 다릅니다. 올바른 주소를 검색해 선택해주세요.",
        ...
        self.s_entry = ctk.CTkEntry(sr, placeholder_text="주소 입력 후 검색",
                                    height=36, width=360)
        ...
        self.s_status = ctk.CTkLabel(body, text="", text_color="gray",
                                     font=ctk.CTkFont(size=11))
        ...
        self.verify_lbl = ctk.CTkLabel(body, text="", font=ctk.CTkFont(size=12),
                                        wraplength=520)
    ...
    def _search(self):
        ...
        if self._items:
            for r in self._items:
                self.lb.insert("end", r['address'])
            self.s_status.configure(text=f"{len(self._items)}건 — 선택 후 '이 주소로 수정' 클릭")
        else:
            self.s_status.configure(text="결과 없음. 다른 검색어를 시도해보세요.")
    ...
    def _on_select(self, _=None):
        ...
        if verdict == '일치':
            self.verify_lbl.configure(text=f"✅ 재검증 완료: {rev}", text_color="#1A8A1A")
        else:
            self.verify_lbl.configure(text=f"⚠️ 재검증: {rev}  (저장되지만 확인 권장)",
                                       text_color="#E67E22")
        self._pending = {'lat': chosen['lat'], 'lon': chosen['lon'],
                         'address': chosen['address'], 'reverse': rev, 'verdict': verdict}
        self.ok_btn.configure(state="normal")

    def _confirm(self):
        if not self._pending:
            messagebox.showwarning("선택 필요", "검색 후 주소를 선택해주세요.", parent=self)
            return
...
class DoneDialog(ctk.CTkToplevel):
    def __init__(self, parent, output_path: str, warn_cnt: int):
        ...
        if warn_cnt:
            ctk.CTkLabel(self,
                         text=f"⚠️  주소 미수정 항목 {warn_cnt}건 — 파일의 '주소검증결과' 열 확인 권장",
    ...
    def _open_folder(path: str):
        """Windows: 탐색기, macOS: Finder 로 폴더 열기"""
...
class App(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        self.api_entry = ctk.CTkEntry(body,
        ...
        ar.pack(fill="x")
        self.addr_entry = ctk.CTkEntry(ar,
        ...
        self.file_lbl = ctk.CTkLabel(fr, text="선택된 파일 없음", text_color="gray",
                                      font=ctk.CTkFont(size=11),
                                      wraplength=430, anchor="w")
        self.file_lbl.pack(side="left", fill="x", expand=True)

        self.run_btn = ctk.CTkButton(body, text="▶  배송 순서 자동 정리 시작",
                                      height=50,
                                      font=ctk.CTkFont(size=15, weight="bold"),
        ...
        self.bar.set(0)
        self.pct_lbl = ctk.CTkLabel(body, text="", font=ctk.CTkFont(size=11),
                                     text_color="gray")
        self.pct_lbl.pack(anchor="e", pady=(0, 10))

        self._label(body, "작업 현황")
        self.log = ctk.CTkTextbox(body, height=230,
        ...
        try:
            ...
            for i, (di, row) in enumerate(df.iterrows()):
                ...
                if verdict not in ('일치', '위치없음', '확인불가'):
                    ...
                    self._step("⚠️ 불일치 — 팝업에서 올바른 주소를 선택해주세요",
                               0.32 + (i + 1) / total * 0.13)
    ...
    def _save_xlsx(self, mapping: dict):
        ...
        [인덱스 처리]
        pandas read_excel 후 dropna 를 거쳐도 인덱스는 reset 되지 않으므로
        mapping key(= df.index) → xlsx 행번호 변환:
            xlsx_row = pandas_index + 2  (헤더 1행 + 0-base→1-base 보정)
        re-read 없이 직접 변환하여 인덱스 불일치 버그를 원천 차단.
        """
        try:
            ...
            # pandas index → xlsx 행번호 직접 변환 (re-read 불필요)
            for df_idx, order_val in mapping.items():
                xlsx_row = df_idx + 2   # pandas 0-base + 헤더 1행 보정
                ws.cell(row=xlsx_row, column=col, value=order_val)
